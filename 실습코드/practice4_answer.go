...
func main(){
	...
	*/
	...
	if totalPrice > 5000{ //5000원 초과이면 10% 할인
		totalPrice = int(float32(totalPrice) * 0.9)
	}

	if totalPrice > 5000 && isTodayMonday{ //5천원 초과이면월요일이면 추가할인
		totalPrice = int(float32(totalPrice) * 0.9)
	}

	if americanoOrder > 4 || latteOrder > 4 { //같은 음료 4잔 이상이면 할인
			totalPrice = int(float32(totalPrice) * 0.9)
	}
	fmt.Println(drinkNames[0],americanoOrder,"잔, ",drinkNames[1], latteOrder,"잔 주문하셨습니다" )
